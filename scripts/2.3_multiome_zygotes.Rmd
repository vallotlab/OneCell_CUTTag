...
here::i_am("scripts/2.3_multiome_zygotes.Rmd")
mainDir = here::here()
...
df <- data.frame(cell <- seurat@meta.data$CB,
                 dna_fragments <- seurat@meta.data$frequency_count,
...
p_count <- ggplot(p_count[[1]][["data"]], aes(x=ident, y=frequency_count)) + 
...
p_count2 <- ggplot(subset(seurat@meta.data, filtering == 'pass'), 
                  aes(x=orig.ident, y=frequency_count, fill=orig.ident)) + 
...
frags_to_rna_reads <- ggplot(seurat_f@meta.data, aes(x = frequency_count, y = nCount_RNA, color = orig.ident)) +
...
frags_to_genes <- ggplot(seurat_f@meta.data, aes(x = frequency_count, y = nFeature_RNA, color = orig.ident)) +
                             geom_point(alpha = 1, stroke = 0, size = 2) +  
                             labs(x = "N unique DNA fragments",
                                  y =  "N unique genes",
...
max_cell <- subset(seurat_zygote5, subset = frequency_count == max(seurat_zygote5@meta.data[["frequency_count"]]))
